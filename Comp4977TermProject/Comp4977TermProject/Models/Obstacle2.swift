...
class Obstacle2: SKSpriteNode {
    private var obstacleTextures: [SKTexture] = []

    init() {
        let spriteSheet = SKTexture(imageNamed: "SLIME_sprite_sheet")
        obstacleTextures = extractFrames(from: spriteSheet, row: 2, columns: 5, totalRows: 5)
    ...
    private func startAnimation() {
        ...
        self.run(SKAction.repeatForever(animation), withKey: "obstacle2Animation")
    }
